---
title: Suspense
description: Learn how suspense works in Vue.
keywords: Vue, style bindings, documentation, Vue app, Vue css transition
---
  
  :::caution[Experimental Freature]
     `<Suspense>` is an experimental feature. It is not guaranteed to reach stable status and the API may change before it does.
   :::

   `<Suspense>` is a built-in component for organizing async dependencies in a component tree. It can render a loading state while waiting for multiple nested async dependencies down the component tree to be resolved.
  ...
  Without `<Suspense>`, each of them will need to handle its own loading / error and loaded states. In the worst case scenario, we may see three loading spinners on the page, with content displayed at different times.
  ...
  2. <a href="https://vuejs.org/guide/components/async.html" class="addcolortext">Async Components</a>
  ...
  When a revert happens, fallback content will not be immediately displayed. Instead, `<Suspense>` will display the previous `#default` content while waiting for the new content and its async dependencies to be resolved.
   ...
   This behavior can be configured with the timeout prop: `<Suspense>` will switch to fallback content if it takes longer than timeout to render the new default content. A timeout value of 0 will cause the fallback content to be displayed immediately when default content is replaced.

  ## Events

  The `<Suspense>` component emits 3 events, this includes:

  1. `pending`: This event occurs when entering a pending state.

  2. `resolve` : This event is emitted when new content has finished resolving in the `default` slot.

  3. `fallback`: This event is fired when the contents of the fallback slot are shown.

  These events could be used, for example, to show a loading indicator in front of the old DOM while new components are loading.
  ...
  In addition, these components are often used in conjunction with the `<RouterView>` component from <a href="https://router.vuejs.org/" class="addcolortext"> Vue Router </a>.
  ...
  Vue Router has built-in support for <a href="https://router.vuejs.org/guide/advanced/lazy-loading.html" class="addcolortext"> lazily loading components </a> using dynamic imports. These are distinct from async components and currently they will not trigger `<Suspense>`. However, they can still have async components as descendants and those can trigger `<Suspense>` in the usual way.
